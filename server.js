...
app.get('/api/fortune', authenticateUser, async (req, res) => {
    ...
    try {
        ...
        try {
            const hitokotoResponse = await axios.get('https://v1.hitokoto.cn/?c=i&encode=json', { timeout: 3000 });
            quote = hitokotoResponse.data.hitokoto;
            quote_source = hitokotoResponse.data.from;
        } catch (hitokotoError) {
            console.error('获取一言API失败:', hitokotoError.message);
        }
        ...
        let luck_level_text = luck_number >= 90 ? '大吉' : (luck_number >= 60 ? '中吉' : '小吉');
